...
from signalrcore.messages.base_message import BaseMessage, BaseMessageEncoder
...
class BaseHubConnection(object):
    ...
    def __init__(self, url, protocol, headers={}, keep_alive_interval=15, reconnection_handler=None, verify_ssl=False):
        ...
        self.connection_checker = ConnectionStateChecker(lambda: self.send(PingMessage()),
                                                         keep_alive_interval)
    ...
    def start(self):
        ...
        self._thread = threading.Thread(
            target=lambda: self._ws.run_forever(sslopt={"cert_reqs": ssl.CERT_NONE} if not self.verify_ssl else {}))
    ...
    def stop(self):
        ...
        if self.reconnection_handler.running and self.reconnection_handler.exception:
            self.logger.debug("__Reconnection FINISHED__")
            self.reconnection_handler.stop()
            self.reconnection_handler.running = False
            self.reconnection_handler.reconnecting = False
    ...
    def on_open(self):
        ...
        self.send(msg)
        if self.reconnection_handler.reconnecting:
    ...
    def on_close(self):
        ...
        if self.connection_checker.running:
            self.connection_checker.stop()

        if self.on_disconnect is not None and callable(self.on_disconnect):
            self.on_disconnect()

        if self.reconnection_handler.running and self.reconnection_handler.reconnecting:
            self.logger.debug("-- web socket re connecting... --")

    def on_error(self, error):
        self.logger.debug("-- web socket error --")
        self.logger.error("{0} {1}".format(error, type(error)))

        if type(error) in [ConnectionRefusedError, ConnectionResetError]:
            if not self.reconnection_handler.running and self.reconnection_handler.reconnecting:
                self.connection_checker.stop()
                self.handshake_received = False
                self.reconnection_handler.start()
                self.state = ConnectionState.reconnecting

        elif type(error) == websocket._exceptions.WebSocketConnectionClosedException:
    ...
    def on_message(self, raw_message):
        ...
        for message in messages:
            ...
            if message.type == MessageType.invocation:
                fired_handlers = list(filter(lambda h: h[0] == message.target,
                                             self.handlers))
            ...
            if message.type == MessageType.completion:
                fired_handlers = list(filter(lambda h: h.invocation_id == message.invocation_id,
                                             self.stream_handlers))
                for handler in fired_handlers:
                    handler.complete_callback(message)

                # unregister handler
                self.stream_handlers = list(filter(lambda h: h.invocation_id != message.invocation_id,
                                                   self.stream_handlers))
    ...
    def send(self, message):
        ...
        if isinstance(message, BaseMessage):
            jsonString = BaseMessageEncoder().encode(message)
            self.logger.debug("Sending message {0}".format(jsonString))
        # self.logger.debug("Sending message {0}".format(message))
        try:
            # if self.state in [ConnectionState.connected, ConnectionState.connecting, ConnectionState.reconnecting]:
            ...
            if self.reconnection_handler is not None and not self.reconnection_handler.running \
                    and self.reconnection_handler.reconnecting:
                self.reconnection_handler.start()
                self.state = ConnectionState.reconnecting
            # Connection closed
    ...
    #        threading.Thread(target=self.deferred_reconnect,
    #            args=(sleep_time,))
    ...
    def stream(self, event, event_params):
        ...
        self.send(StreamInvocationMessage({},
                                          invocation_id,
                                          event,
                                          event_params))
        return stream_obj
